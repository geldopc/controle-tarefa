...
import { GridDataResult, PageChangeEvent, RowArgs, SelectableMode, SelectableSettings } from "@progress/kendo-angular-grid";
...
export class HomePage implements OnInit {
    ...
    ngOnInit() {
        ...
        if (this.ocorrenciaSiapec3) {
            ...
            this.status = { label: "Selecione...", valor: "", cor: "info" };
    ...
    popularSolicitante(item): Tuple[] {
        // console.log('popularSolicitante...', item, this);
        ...
        setTimeout(() => {
            ...
            load.present().then(() => {
                ...
                .then(resp => {
                    ...
                    if (resp && resp.length) {
                        this.populaOcorrencia(resp.filter(a => a.inStatus !== 'TF'), status.valor);
        ...
        if (event.target.files && event.target.files[0]) {
            ...
            reader.onload = (event: any) => {
                ...
                if (event.target.result) {
                    ...
                    anexo.cdUsuarioAtendimento = GerenciadorSessao.usuario.nrDocumento;
